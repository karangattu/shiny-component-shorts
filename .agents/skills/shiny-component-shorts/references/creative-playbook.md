...
- Reserve the top 20% and bottom 20% of the frame for branding; the recorder stamps the Shiny wordmark into the top-left of that band. Center the app in the middle 60% band, use 3–5% side gutters, and stretch its primary card or panel across the available horizontal space; do not use a narrow desktop `max-width` that creates dead space.
