...
## <i class="fa-solid fa-map"></i> This repo, at a glance

![](process.jpg){fig-align="center" height="540"}

::: notes
Orient the audience: the skill (instructions the agent reads), the shared scripts (recorder, validator, TTS, merger), and per-video folders each containing an app, an actions file, and generated artifacts.
:::
...
::: notes
Favorite slide for technical audiences. Non-technical framing: the envelope-flap signature. Technical term: content-addressed approval.
:::
...
## <i class="fa-solid fa-diagram-project"></i> The whole loop, on one slide

![](quality_gate_loop.jpg){fig-align="center" height="540"}

::: notes
Trace one full journey out loud: idea → contract check → app → run it → narration envelope → TTS → measure → timing plan → human gate (hashes) → record (pre-flight, cursor, code card) → validate (timeline vs. windows) → merge (loudnorm) → cost report. Every arrow that points backward is a gate that failed and sent work back.
:::
